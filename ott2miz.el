...
(defun int-or-symbol-name (obj)
  (cond ((symbolp obj) (symbol-name obj))
	((integerp obj) (int-to-string obj))
	(t (error "Don't know how to compute the name of '%s'" obj))))
...
(defun translate-many (indexname)
  ...
  (unless (file-regular-p indexname)
    (error "The index file '%s' is not a regular file!" indexname))
  (save-excursion
    (find-file indexname)
